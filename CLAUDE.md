...
- **No CharacterManager registration; addon driven entirely by direct writes.** Spawned BattleCharas live in `ClientObjectManager` (via `CreateBattleCharacter`) only — never inserted into `CharacterManager._battleCharas`. (We tried hooking `CharacterManager.LookupBattleCharaByEntityId` to expose doppels through the resolver; it crashed in interaction with Reloaded.Hooks's trampoline when other plugins chained `ActorControl` detours upstream. Removed.) `PartyHud` drives the `_PartyList` addon end-to-end: `MainGroup.PartyMembers` writes per frame so `MemberCount > 0` activates the render-members path; `PartyListNumberArray` writes during `AddonLifecycle.PreRequestedUpdate` for `Targetable` / HP / MP / level / class icon / per-slot status icon ids; direct `AtkTextNode.SetText` on each `_statusIcons[s]`'s first text-node child during `PostRequestedUpdate` for the duration countdown text (the addon's own update path produces blank text when the agent can't resolve the BC). Click targeting and mouseover tooltips do **not** work for doppels — both go through `LookupBattleCharaByEntityId` and we don't intercept it. Accepted limitation.
